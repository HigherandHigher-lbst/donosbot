...
RULES_TEXT = (
    ...
    "1. Конфиденциальность: Каждое обращение фиксирует ваш технический ID. Он доступен только администратору. Данные не передаются третьим лицам.\n"
    "2. Целевое использование: Спам и флуд караются. 1 раз: варн, 2 раз: бан.\n"
    "3. Ошибочные доносы: Немедленно сообщите админу, иначе — варн или мут.\n"
    "4. Клевета: Мут 24 часа или бан без права апелляции.\n\n"
...
def check_callback(call):
    if is_subscribed(call.from_user.id):
        ...
        bot.send_message(call.message.chat.id, "Проверка пройдена. Теперь отправьте ваш донос.")
    else:
        bot.answer_callback_query(call.id, "Вы все еще не вступили в группу!", show_alert=True)
...
def handle_private_message(message):
    if not is_subscribed(message.from_user.id):
        ...
        bot.send_message(message.chat.id, "Доступ ограничен. Вступите в группу.", reply_markup=markup)
        return

    if message.text:
        try:
            bot.send_message(TARGET_CHAT_ID, message.text, message_thread_id=TOPIC_ID)
            admin_log = f"Log: New report\nID: {message.from_user.id}\nUser: @{message.from_user.username}"
            bot.send_message(ADMIN_CHAT_ID, admin_log)
            bot.send_message(message.chat.id, "Донесение доставлено.")
        except Exception as e:
            bot.send_message(message.chat.id, "Ошибка отправки. Проверьте права бота.")
